...
export const FocusFlowGame: React.FC<FocusFlowGameProps> = ({ onComplete }) => {
  ...
  const moveTarget = () => {
    ...
    const safeWidth = Math.max(0, gameAreaSize.width - targetSize);
    const safeHeight = Math.max(0, gameAreaSize.height - targetSize);
    
    // Ensure target is at least 20px from edges
    const minDistance = 20;
    const maxWidth = Math.max(0, safeWidth - minDistance);
    const maxHeight = Math.max(0, safeHeight - minDistance);
