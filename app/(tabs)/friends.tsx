...
export default function FriendsScreen() {
  ...
  // Update local messages when selected friend changes
  useEffect(() => {
    if (selectedFriend) {
      const currentFriend = friends.find(f => f.id === selectedFriend.id);
      if (currentFriend) {
        setLocalMessages(currentFriend.messages || []);
      }
    }
  ...
  const handleSendMessage = () => {
    ...
    // Send to store
    if (audioUri) {
      sendMessage(selectedFriend.id, audioUri, true, 'user');
      setAudioUri(null);
    } else {
      sendMessage(selectedFriend.id, messageText, false, 'user');
      setMessageText('');
    }
